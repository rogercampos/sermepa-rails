  ...
  module Rails
    class Transaction
      ...
      def initialize(opts)
        ...
        notify_url = opts[:notify_url] || Sermepa::Rails.notify_url
        url_ok = opts[:url_ok] || Sermepa::Rails.ok_url
        url_error = opts[:url_error] || Sermepa::Rails.error_url
        ...
        @fuc = validate_fuc Sermepa::Rails.fuc
        @term_number = validate_term_number Sermepa::Rails.term_number
        @currency = validate_currency Sermepa::Rails.term_currency
      ...
      def signature
        raise ArgumentError, "Key type must be :complete or :expanded" unless [:complete, :expanded].include?(Sermepa::Rails.key_type)
        key = "#{@amount}#{@order}#{@fuc}#{@currency}"
        if @transaction_type == "5" && @sum_total
          key << @sum_total
        end
        if Sermepa::Rails.key_type == :expanded
          key << "#{@transaction_type}#{@notify_url}"
        end
        key << Sermepa::Rails.encrypt_key
        SHA1.new(key).to_s
      end
